...
<div id="allContainer" class="ac">
 <div id="logo">
        ...
        <div id="pgButtons" class="littleDD">
	        <a href="http://192.168.43.132/ibis/IBISmain.html" class="buttonclass littleDD"><img src="" alt="">Back to Main Screen</a></div></div>
...
<?php
 ...
 $username = getUserName("$cleanname", "$securityA");
  ...
  if ($emailCount > 0){
    ...
    $stmt2->close();
   // print "statement 2 closed without a hitch";
    ...
    //print "statement created...start bind param now";
   ...
   // $name = $_POST['name']; 
    ...
    $undescore = '_';
    $role = "c";
   // $cleanname = str_replace("$space", "$undescore", $name);
    $lastname = $_POST['lastname'];
    $email = $_POST['email']; 
    //$media_references = $_FILES['picture']['name']; 
    $mediaRef = $fileList;
    $regDate = $uploadDate;
    $securityQ = $_POST['secQ'];
    //$securityA = $_POST['secA']; 
   // $username = getUserName("$cleanname", "$securityA");
