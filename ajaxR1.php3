...
$mcount = 0;
  ...
  if ($mysqli->connect_error){
    ...
    $justname = $_FILES['uptest']['name'];
	$tmpFilePath = $_FILES['uptest']['tmp_name'];
	$prefix = $_POST['atag']; 
	//$prefix = "anim";
	$MAEmetricPair = "";
	$metricList =  "";
	...
	foreach ($pairArray as $pairSet) {
	  ...
	  $MSEmetricA = explode(" ",$MSEmetric );
	  $MSE = ceil($MSEmetricA[0]);
	
	 $RMSEmetric =  `/usr/bin/compare -metric RMSE $uploadfile $theSPath null: 2>&1`;
	 $RMSEmetricA = explode(" ",$RMSEmetric );
	 $RMSE = ceil($RMSEmetricA[0]);
	 
	 $MAEmetric =  `/usr/bin/compare -metric MAE $uploadfile $theSPath null: 2>&1`;
	 $MAEmetricA = explode(" ",$MAEmetric );
	 $MAE = ceil($MAEmetricA[0]);
	 $metricAv = ceil(($MSE + $RMSE + $MAE) /3);
	...
	// $metricAvg = ($MAEmetric + $AEmetric + $MAEmetric + $MEPPmetric  + $NCCmetric + $PAEmetric) / 6;
	  ...
	  */
	  $metricList = "Metric Avg: $metricAv \n";
...
<html lang="EN" dir="ltr" xmlns="http://www.w3.org/1999/xhtml">
    <head>
     ...
     <body> <p>'.$count.' files processed</br>'.$mcount.' files match criteria</p>';
	$htmlF = '</body></html>';
